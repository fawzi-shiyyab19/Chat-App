...
export default function Chat() {
  ...
  const connect = async (name) => {
    await axios
      .post("http://localhost:4000/user", { user: name.toLowerCase() })
      .then(async (res) => {
  ...
  const leaveHandler = async (e) => {
    e.preventDefault();
    await axios.post(
      `http://localhost:4000/user/${localStorage.getItem("name")}`
    );
    setJoined(false);
  ...
  const sendHandler = async (e) => {
    ...
    await axios.post("http://localhost:4000/messages", { name, message });
